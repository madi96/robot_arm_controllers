...
class ButtonPrimitive: public Primitive {
...
// For BAXTER
        ...
        bool planAndExecute(std::vector<double> goal,
            ...
            moveit::planning_interface::MoveGroupInterface::Plan to_approach_point_plan,
                   wrist_rotation_plan,to_goal_plan;
            bool planning_result = false;
            bool execution_result = false;
            ...
            while(!planning_result && random_frame_count < random_frame_budget ) {
                ...
                planning_result = planForTrajectory(to_approach_point_plan, wrist_rotation_plan, to_goal_plan, random_frame_count);
                random_frame_count +=1;
            }
            if (planning_result) {
                execution_result = executePlannedMotion(to_approach_point_plan, wrist_rotation_plan, to_goal_plan);
                if (execution_result) {
